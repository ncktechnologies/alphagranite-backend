...
from src.app.interface.generated_schemas import InstallCompletion, ResurfaceScheduling, InstallScheduling, Revision
...
def _coalesce_shop_est_completion_date(
    ...
    """Use stored shop estimate date first; otherwise derive from latest shop plan scheduled end."""
    ...
    try:
        return latest_plan_end.date().isoformat() if latest_plan_end else None
    except Exception:
        return None
...
@router.get("/fabs/cnc-widget")
async def get_fabs_for_cnc_widget(
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(100, ge=1, le=1000, description="Number of records to return"),
    job_id: Optional[int] = Query(None, description="Filter by job ID"),
    fab_type: Optional[str] = Query(None, description="Filter by fab type"),
    ...
    if current_stage:
        ...
        stage_totals_query = stage_totals_query.where(cnc_widget_filter)
