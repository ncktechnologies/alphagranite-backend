...
async def create_shop_plans(
    ...
    try:
        ...
        for stage in plan_data.stages:
            ...
            # Keep one operator per stage; duplicate planning sections are allowed on create.
            if len(stage.operator_ids) > 1:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=(
                        "Only one operator is allowed per stage. "
                        "Each stage can have only one operator."
                    )
                )
    ...
    for other_plan, existing_workstation in conflicting_plans:
        ...
        if existing_workstation.attendance_required or new_workstation.attendance_required:
            ...
            raise HTTPException(
                ...
                detail=(
                    "Cannot create or assign shop plan: Attendance is required for this workstation. "
                    f"Workstation: {required_workstation.name} (ID {required_workstation.id}). "
                    f"Existing FAB {other_plan.fab_id} is using it during {existing_time_range}. "
                    f"Requested FAB {fab_id} time range: {requested_time_range}."
                ),
            )
